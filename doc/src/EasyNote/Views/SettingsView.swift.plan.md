...
- System Reminders operations are consumed through `SystemReminderAgentProviding` and async/throws `SystemReminderWritingProviding`; Settings owns a cancellable reminder task and sequentially reconciles current todos without `DispatchGroup` callback state.
