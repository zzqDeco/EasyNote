...
After a successful save, cleanup considers only direct Documents children with supported audio extensions and EasyNote-managed `recording_` or `restored_recording_` prefixes. Referenced files, arbitrary user files, unsupported extensions, nested files, and directories are preserved. Repeated import overwrites the deterministic asset path and must not create UUID-suffix orphan files.
