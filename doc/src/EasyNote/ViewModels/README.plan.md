...
- `ChatSessionViewModel` owns persisted chat sessions, explicit-session message writes, session-scoped request tasks, cancellation, and retryable failures. Session deletion explicitly stages every related message delete before the session delete; one failed save cancels those staged deletes and restores the relationship without invoking SwiftData's crashing Xcode 16.4 versioned-relationship rollback path. Session navigation preserves completed failures, destructive mutations cancel active requests only after their save succeeds, and retries re-save a user message when the original failure happened before it persisted. A retry keeps its previous failure until a successful response or replacement failure completes.
