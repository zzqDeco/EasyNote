...
- An ordinary refresh failure retains the currently published sessions and selection; it does not hide persisted chats or create a speculative default session.
- Mutations initiated by delayed UI actions carry a session UUID and resolve the live model from the current context before editing.

## Tests

- `ChatResponseIntegrityTests` covers session capture, cancellation, retry, deletion, context rebuilding after failed message/title/clear/create saves, refresh failure preservation, UUID-based title retry, and concurrent session requests.
