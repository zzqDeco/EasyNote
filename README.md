...
- AI：DeepSeek 聊天补全接口用于日记摘要、文本润色、扩写、总结和笔记探索；生成结果先进入当前会话历史，用户确认后才应用到日记或转写内容。笔记探索请求固定绑定发起时的会话；服务失败时只返回可由当前日记验证的本地结果，无匹配事实时显示可重试错误。
...
3. 在应用的“设置”页填写并保存 DeepSeek API 密钥，再阅读隐私提示并明确允许发送所选文本。未配置密钥或未授权时，AI 调用都会在本地失败，不会发起网络请求。
...
DeepSeek API 密钥存储在本机钥匙串中，使用 `WhenUnlockedThisDeviceOnly` 可访问级别，不会同步到其他设备。AI 内容授权默认关闭，保存或迁移已有密钥不会自动授权；授权后，只有用户选择处理的日记或语音转写文本会发送给 DeepSeek。录音音频文件不会发送，API 密钥仅通过 HTTP Authorization 请求头用于 DeepSeek 鉴权，不会写入提示正文。用户可以随时在设置中撤回授权，撤回后后续 AI 请求会在联网前被阻止。
