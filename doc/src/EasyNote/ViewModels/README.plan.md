...
- `ChatSessionViewModel` owns persisted chat sessions, explicit-session message writes, session-scoped request tasks, cancellation, and retryable failures. Session deletion fetches and deletes the session plus only messages not referenced by another legacy session in a short-lived `ModelContext`; one failed save discards that context without mutating the UI context or invoking SwiftData's crashing Xcode 16.4 versioned-relationship rollback path. Session navigation preserves completed failures, destructive mutations cancel active requests only after their save succeeds, and retries re-save a user message when the original failure happened before it persisted. A retry keeps its previous failure until a successful response or replacement failure completes.
- `ChatResponseGenerator` maps provider results to provider/local/failure/cancelled outcomes; `LocalDiaryQueryAnalyzer` is the only offline fallback and may use only captured diary facts. Query command and mention-question phrases are removed at term boundaries rather than as arbitrary substrings, matching includes explicit mood metadata and single-character CJK topics, and scoped recent or mood summaries filter by topic before aggregation while natural broad mood prompts remain unscoped.
