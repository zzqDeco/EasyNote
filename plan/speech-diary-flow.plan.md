...
- `SpeechRecognitionService` writes audio buffers to a local file during recording so saved diary entries only receive existing recording URLs, tears down the audio pipeline on file setup/write/start failures, preserves recording errors after recognition cancellation callbacks, ignores stale recognition callbacks from previous sessions, and deletes stale unclaimed recording files before starting a replacement recording.
- New-entry views capture active or finished recordings, delete abandoned or superseded captured `.caf` files on cancel/dismiss or re-record, existing diary entries delete replaced `.caf` or legacy `.m4a` recording files only after the new recording reference saves successfully, and diary deletion removes its saved local recording file after the model delete saves.
...
- Unit tests cover preserving an existing recording error when recognition completion callbacks arrive after teardown.
- Unit tests cover rejecting stale speech-recognition callback sessions.
- Unit tests cover deleting legacy `.m4a` recording files.
