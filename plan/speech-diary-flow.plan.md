...
- `SpeechRecognitionService` writes audio buffers to a local file during recording so saved diary entries only receive existing recording URLs, tears down the audio pipeline on file setup/write/start failures, preserves recording errors after recognition cancellation callbacks, and deletes stale unclaimed recording files before starting a replacement recording.
