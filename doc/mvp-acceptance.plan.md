...
- with a configured or migrated key and no prior consent, confirm the first-use disclosure states that selected diary/transcription text and recent diary fields used for recommendations may be sent to DeepSeek, while audio files are not sent and the key is only used for authentication
