...
- 设置：主题模式、主题色、本机 API key 配置、本地数据备份和待办提醒方式。
...
如果本地 SwiftData 数据库无法打开，EasyNote 会停留在恢复界面，不会自动清空数据。可以先重试；选择重建时必须再次确认，应用会先把 `EasyNote.store` 及现有 WAL/SHM 文件复制到 `Documents/EasyNoteRecovery/<timestamp>`，再创建新数据库。
