...
- `ChatSessionViewModel` owns persisted chat sessions, explicit-session message writes, session-scoped request tasks, cancellation, and retryable failures. Session deletion stages every related message delete and the session delete in a dedicated undo group; one failed save undoes that group without invoking SwiftData's crashing Xcode 16.4 versioned-relationship rollback path. Session navigation preserves completed failures, destructive mutations cancel active requests only after their save succeeds, and retries re-save a user message when the original failure happened before it persisted. A retry keeps its previous failure until a successful response or replacement failure completes.
