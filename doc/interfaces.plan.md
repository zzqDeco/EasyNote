...
Only messages referenced by exported chat sessions are included in `sessionMessages`; fetchable orphaned messages from deleted sessions are not exported. A message ID may belong to only one chat session. Export normalizes legacy cross-session message references by assigning duplicate references new backup-only UUIDs and copying their message payload, preserving both conversations while satisfying this contract. Decode and import apply the same normalization to older V1 files before strict validation, so previously accepted shared-message backups remain restorable without recreating unsupported shared SwiftData relationships.

`BackupLimits.default` bounds raw files at 64 MiB, one audio asset at 16 MiB, aggregate decoded audio at 47 MiB to reserve base64 envelope headroom, diaries at 10,000, todos at 50,000, chat sessions at 5,000, session messages at 100,000, and audio assets at 500. Raw byte count is checked before JSON decode. Entity counts, declared/decoded audio byte equality, per-asset bytes, aggregate audio bytes, references, identifiers, and extensions are checked after decode and before any SwiftData or destination-file mutation. Export fetches at most each model limit plus one and stops the audio-file loop at the asset limit plus one before rejecting an oversized store. Unsupported versions, duplicate IDs unrelated to legacy shared messages, missing references, malformed base64, and resource-limit failures leave models and destination files unchanged.

The backup service protocol is async. Export snapshots only DTO values and relationships on `MainActor`; JSON encode/decode and audio reads/writes run off the main thread. Import stages audio under a temporary transaction directory, then installs each asset at deterministic `restored_recording_<asset-id>.<ext>` paths. A canonical target referenced by a diary absent from the backup is protected and the imported asset uses a stable `_imported` collision path. Existing replaceable targets are copied into rollback storage before overwrite, then restored by move or atomic replacement without another full-file copy. SwiftData changes are applied and saved on `MainActor`; a failed save rolls back both the `ModelContext` and installed audio files. Same-ID model records are updated, missing same-type records are inserted, and local records absent from the backup are preserved. When an older backup is imported over a session with newer local messages, those local messages remain attached and the session modified time stays at the latest imported, existing, or preserved message timestamp.

After a successful save, cleanup considers only direct Documents children with supported audio extensions and the backup-owned `restored_recording_` prefix. Recorder-owned `recording_` files, referenced files, arbitrary user files, unsupported extensions, nested files, and directories are preserved. Repeated import overwrites the deterministic asset path and must not create UUID-suffix orphan files.
