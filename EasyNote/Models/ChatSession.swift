...
extension EasyNoteSchemaV1 {
    ...
    final class ChatSession {
        ...
        var messages: [EasyNoteSchemaV1.SessionMessage] = []

        init(
            id: UUID = UUID(),
            title: String = "新会话",
            messages: [EasyNoteSchemaV1.SessionMessage] = []
        ) {
            self.id = id
            self.title = title
        ...
        // 添加消息到会话
        func addMessage(_ message: EasyNoteSchemaV1.SessionMessage) {
        ...
        // 通过ID查找消息
        func findMessage(withID id: UUID) -> EasyNoteSchemaV1.SessionMessage? {
...
typealias ChatSession = EasyNoteSchemaV1.ChatSession
typealias SessionMessage = EasyNoteSchemaV1.SessionMessage
